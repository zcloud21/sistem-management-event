          ...
          <form id="update-voucher-form-{{ $voucher->id }}" action="{{ route('vouchers.update', $voucher->id) }}" method="POST" class="needs-confirmation" data-confirmation-title="Konfirmasi Pembaruan Voucher" data-confirmation-message="Apakah Anda yakin ingin menyimpan perubahan pada voucher '{{ $voucher->code }}'?">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <x-text-input id="code" class="block mt-1 w-full font-mono uppercase" type="text" name="code" :value="old('code', $voucher->code)" required autofocus />
              ...
              <div>
                ...
                <select name="type" id="type" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                  <option value="fixed" {{ old('type', $voucher->type) == 'fixed' ? 'selected' : '' }}>Fixed (Potongan Tetap Rp)</option>
                  <option value="percentage" {{ old('type', $voucher->type) == 'percentage' ? 'selected' : '' }}>Percentage (Potongan %)</option>
                </select>
              ...
              <div>
                ...
                <x-text-input id="value" class="block mt-1 w-full" type="number" name="value" :value="old('value', $voucher->value)" required placeholder="Cth: 100000 atau 10" />
              ...
              <div>
                ...
                <x-text-input id="expires_at" class="block mt-1 w-full" type="date" name="expires_at" :value="old('expires_at', $voucher->expires_at ? \Carbon\Carbon::parse($voucher->expires_at)->format('Y-m-d') : '')" />
              ...
              <div>
                ...
                <x-text-input id="max_uses" class="block mt-1 w-full" type="number" name="max_uses" :value="old('max_uses', $voucher->max_uses)" min="1" />
